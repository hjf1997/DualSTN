...
class BaseModel(ABC):
    ...
    def inverse_norm(self, x):
        ...
        if type(self.opt.mean) == torch.Tensor:
            self.opt.mean = self.opt.mean.to(x.device)
        if type(self.opt.std) == torch.Tensor:
            self.opt.std = self.opt.std.to(x.device)
        return x * self.opt.std + self.opt.mean
